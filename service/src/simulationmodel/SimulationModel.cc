...
void SimulationModel::scheduleTrip(const JsonObject& details) {
  ...
  if (receiver && package) {
    ...
    Package* pkg2;
    Package* pkg3;
    pkg1->setPriority(new ExpeditedShipping());
    pkg2->setPriority(new StandardShipping());
    pkg3->setPriority(new NoRushShipping());
    scheduledDeliveries.push_back(pkg1);
    scheduledDeliveries.push_back(pkg2);
    std::cout << scheduledDeliveries.size() << std::endl;
    scheduledDeliveries.push_back(pkg3);
    sortScheduledDeliveries();
    std::cout<<scheduledDeliveries[0]<<std::endl;
    std::cout<<scheduledDeliveries[1]<<std::endl;
    std::cout<<scheduledDeliveries[2]<<std::endl;
    controller.sendEventToView("DeliveryScheduled", details);
  }
}

void SimulationModel::sortScheduledDeliveries() {
  std::sort(scheduledDeliveries.begin(), scheduledDeliveries.end(), [](Package* a, Package* b) {
